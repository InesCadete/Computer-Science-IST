...
N_L corresponde ao numero da linha e a Linha e a lista com numeros de pontes
Ilhas da linha corresponde a lista  de ilhas ordenada da esquerda para a direita*/
...
/* ilhas(Puz, Ilhas)
Puz e um puzzle(lista com cada linha)
Ilhas e a lista de ilhas ordenada
...
Ilhas e a lista de ilhas de um puzzle e Ilha e uma dessas ilhas
lista de Vizinhas e a lista com as ilhas mais proximas
(ordenada de cima para baixo e da esquerda para a direita) .*/
...
/* estado(Ilhas, Estado)
Estado e a lista ordenada das entradas 
referentes a cada uma das ilhas de Ilhas.*/
...
Posicoes corresponde a lista de posicoes entre Pos1 e Pos2
sendo que Pos1 e Pos2 estao na mesma linha ou coluna
(se tal nao acontecer o resultado e false) */
...
Ponte e uma ponte cujos extremos sao Pos1 e Pos2,
(as duas posicoes de Ponte estao ordenadas).*/

%se x(linha) for igual
...
Determina(resultado verdadeiro ou falso)
se a adicao da ponte(Pos1, Pos2) nao faz
com que I (ilha) e Vz (vizinha de ilha) deixem de ser vizinhas.*/
...
Nova_Entrada e a entrada modificada na lista de vizinhas,
removendo as ilhas que deixaram de ser vizinhas depois da adicao da ponte entre Pos1 e Pos2*/
...
Novo_estado e o estado depois de atualizar as ilhas vizinhas
de cada uma das suas entradas, sendo que foi criada uma ponte entre Pos1 e Pos2*/
...
Ilhas_term e a lista de ilhas ja com todas as pontes associadas,
que acontece se N_pontes for diferente de "X" e o comprimento da lista
Pontes for N_pontes .*/
...
Nova_entrada resulta de remover da lista de ilhas vizinhas da Entrada
as ilhas de Ilhas_term,*/

tira_ilhas_terminadas_entrada(Ilhas_term, [ilha(N_pontes,Pos),[Cab|Resto], Pontes], Nova_entrada) :-
    tira_ilhas_terminadas_entrada(Ilhas_term, [ilha(N_pontes,Pos),[Cab|Resto], Pontes], [], Nova_entrada).

tira_ilhas_terminadas_entrada(_, [ilha(N_pontes,Pos), [], Pontes], Acc, [ilha(N_pontes,Pos), Acc, Pontes]).

%modifica a lista de vizinhas, verificando se cada elemento e ilha terminada
...
Novo_estado e o Estado apos aplicar o predicado tira_ilhas_terminadas_entrada
a cada uma das suas entradas*/
...
Nova_entrada e a Entrada apos substituir o numero de pontes
das ilhas terminadas por "X"*/
...
Novo_estado e o Estado depois de aplicar o predicado marca_ilhas_terminadas_entrada
a cada uma das suas entradas.*/
...
(ilhas_terminadas sao as ilhas terminadas do Estado)*/
...
Novo_estado e o Estado modificado adicionando Num_pontes pontes
entre Ilha1 e Ilha2 .*/

%funcao auxiliar que adiciona pontes a lista Pontes
%caso em que Num_pontes e 1
