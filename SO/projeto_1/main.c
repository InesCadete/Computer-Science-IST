...
pthread_mutex_t simultaneous_backups_mutex = PTHREAD_MUTEX_INITIALIZER; // Mutex initialization


// Global variables to replace MAX_BACKUP and MAX_THREADS
int max_backup = 0;
int max_threads = 0;
...
int main(int argc,char *argv[]) {
  ...
  max_backup = atoi(argv[2]);   // Parse the max_backup argument
  max_threads = atoi(argv[3]); // Parse the max_threads argument

  if (max_backup <= 0 || max_threads <= 0) {
        fprintf(stderr, "Error: max_backup and max_threads must be positive integers.\n");
        return 1;
    }
  ...
  pthread_mutex_destroy(&simultaneous_backups_mutex);
