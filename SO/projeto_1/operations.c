...
extern pthread_mutex_t simultaneous_backups_mutex;

extern int max_backup;
...
void kvs_wait_backup() {
    ...
    pthread_mutex_lock(&simultaneous_backups_mutex); // Lock the mutex

    simultaneous_backups --; // Decrement the active backup counter
    pthread_mutex_unlock(&simultaneous_backups_mutex); // Unlock the mutex
...
int kvs_backup(char* dir_name, struct dirent* entry, char *in_path, char *out_path) {
    ...
    if(simultaneous_backups >= max_backup ) {
      pthread_mutex_unlock(&simultaneous_backups_mutex); // Unlock before waiting
      kvs_wait_backup();
      pthread_mutex_lock(&simultaneous_backups_mutex); // Re-lock the mutex
